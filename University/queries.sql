...
SELECT Name FROM student,enrolled WHERE student.student#=enrolled.student# AND enrolled.subject#='COMP9311';

--(2) Which students have lectures ON Mondays?

SELECT Name, FROM student, enrolled WHERE student.student#=enrolled.student# AND enrolled.subject# in (SELECT subject FROM class WHERE meetsat LIKE 'Monday%' );

--(3) How many students are enrolled in COMP1011?

SELECT count(Name) FROM student,enrolled WHERE student.student#=enrolled.student# AND enrolled.subject#='COMP1011';
...
SELECT count(student#) FROM marks WHERE subject#='COMP2031' AND period='98s2' AND mark <= 60;

--(6) What was the average mark in COMP9311 in 98s2?

SELECT AVG(mark) FROM marks WHERE subject#='COMP9311' AND period='98s2';
...
SELECT  m.mark, m.subject#,m.period,s.Name,m.student# FROM marks m,subject s,student s1 WHERE m.student#=s1.student# AND s.id=m.subject# ORDER BY s1.Name;